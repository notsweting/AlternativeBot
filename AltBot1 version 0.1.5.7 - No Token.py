...
bot_version = 'Version 0.1.5.7.1 [BETA]'
...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
      await ctx.send('This command is nonexistent. Please use a proper command as documented in /help.')
        ...
        print(error)
        embed = discord.Embed(title="Oh noes!", description="There's been an issue!", colour=ctx.author.color, timestamp=ctx.message.created_at)
        embed.add_field(name='Your command raised an exception:', value=f'''```python
{error}```
        The issue has been sent to {developer}!''')
...
@client.command()
async def help(ctx):
    embed = discord.Embed(title="AltBot1 Help and Documentation", description="Basic commands", colour=ctx.author.color, timestamp=ctx.message.created_at)
    ...
    embed.add_field(name='/whois (person)', value='Get info on a member.')
    ...
    embed.add_field(name='/rules', value='Self-explanatory.')
    embed.add_field(name='/dev_update', value='Info about the latest update to the bot!')
    embed.set_footer(text= f'Requested by {ctx.author} | (required) [optional] | Developed by {developer} | {bot_invite} | bot_version', icon_url=ctx.author.avatar_url)
    await ctx.send (embed=embed)
...
async def about (ctx):
    ...
    embed.add_field(name='Programming Language and library', value="discord.py version 1.3.2")
...
> -Made /whois better
> -Made /help better
> -/rules is now a thing
> -/invite is more streamlined
> -/dev_update is now a thing
> -/8ball no longer requires a question
> 
> **Special stuff**
> -We out of alpha wahoo''')
...
async def whois(ctx, member: discord.Member = None):
    ...
    embed.set_footer(text= f'Requested by {ctx.author} | Developed by {developer} | {bot_invite} | {bot_version}', icon_url=ctx.author.avatar_url)
